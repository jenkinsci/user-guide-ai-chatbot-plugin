...
class UserEntity(Base):
    ...
    chats = relationship("ChatEntity", back_populates="user", cascade="all, delete-orphan")
...
class ChatEntity(Base):
    ...
    qa_pairs = relationship("QAPairEntity", back_populates="chat", cascade="all, delete-orphan")
...
class QAPairEntity(Base):
    ...
    question = relationship("QuestionEntity", back_populates="qa_pair", uselist=False, cascade="all, delete-orphan")
    
    # Relationship: 1 QAPair -> 1 Answer
    answer = relationship("AnswerEntity", back_populates="qa_pair", uselist=False, cascade="all, delete-orphan")
...
class QuestionEntity(Base):
    ...
    qa_pair_id = Column(Integer, ForeignKey("qa_pairs.id", ondelete="CASCADE"), unique=True, nullable=False)
