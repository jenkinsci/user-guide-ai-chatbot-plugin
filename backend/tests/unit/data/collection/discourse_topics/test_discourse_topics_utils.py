...
from data.collection.discourse_topics.models import SearchFilters, Post, TopicDetails, SolutionPost
from data.collection.discourse_topics.discourse_topics_utils import build_discourse_endpoint, build_topic_tree, export_topics_to_json

BASE = "https://discourse.example.com"
 
 
@pytest.mark.parametrize("category_slug, subcategory_slug, tag_slug, expected", [
...
def test_base_url_trailing_slash_stripped():
    ...
    filters = SearchFilters(category_slug="support", subcategory_slug=None, tag_slug=None)
...
@pytest.mark.parametrize("raw_posts, expected_root_count, expected_replies", [
...
@pytest.mark.parametrize("topics, expected_length, expected_titles", [
    ...
    (
        [make_topic(1, "Topic A"), make_topic(2, "Topic B"), make_topic(3, "Topic C")],
        3,
    ...
    (
        [TopicDetails(
            topic_id=99,
            title="With Content",
            posts=[Post(id=1, post_number=1, username="alice", content="Hello",
                        is_solution=False, reply_to_post_number=None,
                        created_at="2024-01-01", reactions=[], url="/t/1")],
            solutions=[SolutionPost(id=1, content="The answer", reactions=[], created_at="", topic_id=1, url="", username="mario")],
        )],
        1,
        ["With Content"],
    ),
