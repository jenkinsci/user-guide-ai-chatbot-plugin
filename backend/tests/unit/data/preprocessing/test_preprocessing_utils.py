...
def test_split_doc_types(capsys):
    ...
    assert "https://www.jenkins.io/doc/developer/extensions/pipeline-input-notification/" in dev_urls
...
def test_remove_container_by_class():
    html_content = '<div><div class="keep-me">Safe</div><div class="remove-me">Deleted</div></div>'
...
@pytest.mark.parametrize("html_input, text_to_keep, text_to_remove", [
    ...
    # 4. Multiline comment
    ("<p>Top</p><!--\n  line one\n  line two\n--><p>Bottom</p>", "Bottom", "line one"),
 
    # 5. No comments at all — content must be fully preserved
    ("<article><p>Just text</p></article>", "Just text", None),
])
def test_remove_html_comments(html_input: str, text_to_keep: str, text_to_remove: str | None):
    # Execute the function
...
@pytest.mark.parametrize("html_input, expected_length", [
...
@pytest.mark.parametrize("html_input, expected_output_contains, expected_output_excludes", [
...
def test_strip_html_body_wrappers(html_input: str, expected_output_contains: str, expected_output_excludes: list[str]):
    # Execute the function
